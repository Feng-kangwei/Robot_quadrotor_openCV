  ...
  while(1)
	{
		...
		//配置传感器
		spl06_pressure_rateset( 8 , 4 );//pressure 64 samples per sec , 32 times over sampling		
		spl06_temperature_rateset( 8 , 4 );//temperature 128 samples per sec , 2 times over sampling		
		tx_buf[0] = 0x08;
...
ExtSPL06Detected:
	...
	while(1)
	{
		...
		uint8_t rt = 0;		
		volatile int32_t buf32[2];
		volatile double pressure,temperature;
		...
		uint32_t rx_buf2 = *(uint32_t*)&rx_buf[0];
		uint32_t rx_buf3 = *(uint32_t*)&rx_buf[3];
		buf32[0] =  __REV(rx_buf2)>>8;
		buf32[1] = __REV(rx_buf3)>>8;
		...
		pressure = SPL06_coefficients.c00 + fPsc * qua2 + fTsc * SPL06_coefficients.c01 + qua3;
		temperature = SPL06_coefficients.c0*0.5 + SPL06_coefficients.c1*fTsc;		
		//更新传感器数据
		if( pressure > 0 )
		{
