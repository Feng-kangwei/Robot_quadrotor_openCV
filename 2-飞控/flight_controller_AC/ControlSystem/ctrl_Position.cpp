...
/*控制接口*/
	...
	/*水平位置*/
		...
		/*直线飞行*/
			...
			bool Position_Control_get_LineFlightDistance( double* distance, double TIMEOUT )
			{
				...
				if( LockCtrl(TIMEOUT) )
				{
					...
					*distance = safe_sqrt(e1r.get_square());
		...
		/*设定自动飞行速度*/
			...
			bool Position_Control_reset_XYAutoSpeed( double AtVelXY, double TIMEOUT )
			{
			...
			bool Position_Control_reset_XYZAutoSpeed( double AtVelXYZ, double TIMEOUT )
			{
